...
// import org.onosproject.rest.WebApplicationService;
...
@Component(
        ...
        enabled = true
	// service = QosPolicyService.class   // ← THIS is what makes get() work
)
...
public class L2BridgingComponent {
    ...
    private final Set<MacAddress> blockedHosts = new HashSet<>();

    Map<Integer, String> qosPolicy = new HashMap<>();
   ...
   // @Reference(cardinality = ReferenceCardinality.MANDATORY)
   // protected WebApplicationService webApplicationService;
	...
	// webApplicationService.registerWebApplication(new IntentWebApplication());
        // Schedule set up of existing devices. Needed when reloading the app.
        mainComponent.scheduleTask(this::setUpAllDevices, INITIAL_SETUP_DELAY);
	
	// for (Device device : deviceService.getDevices()) {
    	//     installVideoRule(device.id());
	// }

	qosPolicy.put(1, "IngressPipeImpl.set_high_priority");
	qosPolicy.put(2, "IngressPipeImpl.set_medium_priority");
	qosPolicy.put(0, "IngressPipeImpl.set_low_priority");

	for (Device device : deviceService.getDevices()) {
            installQoSEntries(device.id());
    	}
			
	// blockedHosts.add(MacAddress.valueOf("00:00:00:00:00:1A"));
	// blockedHosts.add(MacAddress.valueOf("00:00:00:00:00:1B"));
	// log.info("Blocked hosts policy initialized: {}", blockedHosts);
        log.info("Started");
    }

    @Deactivate
    protected void deactivate() {
        AppInstance.component = null;   // ← unregister
	deviceService.removeListener(deviceListener);
        hostService.removeListener(hostListener);
	
	// webApplicationService.unregisterWebApplication(new IntentWebApplication());
        log.info("Stopped");
    }
    ...
    public class InternalDeviceListener implements DeviceListener {
        ...
        public void event(DeviceEvent event) {
		    ...
		    installQoSEntries(deviceId);

		    if (!isSpine(deviceId)) {
			setUpDevice(deviceId); // Then do leaf-specific bridging
    		    }

		    // installVideoRule(deviceId);
                });
            }
        }
    }
    ...
    public class InternalHostListener implements HostListener {
		...
		log.info("Policy check for host {}", mac);
    		if (shouldBlockHost(mac)) {
		    log.info("Blocking host {}", mac);
    		    installBlockRule(mac, deviceId);
        	    return;
    		}
    ...
    private boolean shouldBlockHost(MacAddress mac) {
        return blockedHosts.contains(mac);
    }

    private void installBlockRule(MacAddress mac, DeviceId deviceId) {
	PiTableId tableId = PiTableId.of("IngressPipeImpl.acl_table");
	...
	flowRuleService.applyFlowRules(rule);
        ...
        log.info("Installed block rule in ACL table for {}", mac);
    }

    private void installVideoRule(DeviceId deviceId) {

	log.info("Installing video rule on device {}", deviceId);
	PiCriterion match = PiCriterion.builder()
            .matchExact(PiMatchFieldId.of("local_metadata.l4_dst_port"), 8801)
            .build();

	PiAction action = PiAction.builder()
            .withId(PiActionId.of("IngressPipeImpl.set_video_priority"))
            .build();

	FlowRule flowRule = DefaultFlowRule.builder()
            .forDevice(deviceId)
            .forTable(PiTableId.of("IngressPipeImpl.qos_table"))
    ...
    public String getCurrentPolicy() {
    	String action = qosPolicy.getOrDefault(1, "");
    	return action.contains("high") ? "voice" : "video";
    }
        
    public void setVoicePriority() {
    	qosPolicy.put(1, "IngressPipeImpl.set_high_priority");
    	qosPolicy.put(2, "IngressPipeImpl.set_medium_priority");
    	qosPolicy.put(0, "IngressPipeImpl.set_low_priority");

    	applyIntentPolicy();   // 🔥 reapply rules
    }

   
    public void setVideoPriority() {
    	qosPolicy.put(2, "IngressPipeImpl.set_high_priority");
    	qosPolicy.put(1, "IngressPipeImpl.set_medium_priority");
    	qosPolicy.put(0, "IngressPipeImpl.set_low_priority");

    	applyIntentPolicy();
    }

    private void applyIntentPolicy() {
    ...
    private void installQoSEntries(DeviceId deviceId) {

    	// Example: VOICE priority intent
	for (Map.Entry<Integer, String> entry : qosPolicy.entrySet()) {
            installEntry(deviceId, entry.getKey(), entry.getValue());
    	}
    }  


    private void installEntry(DeviceId deviceId, int trafficClass, String actionName) {

    	PiCriterion match = PiCriterion.builder()
            .matchExact(PiMatchFieldId.of("local_metadata.traffic_class"), trafficClass)
            .build();
