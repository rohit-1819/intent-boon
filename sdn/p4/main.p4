...
header ipv6_ext_t {
    bit<8> next_hdr;
    bit<8> hdr_ext_len;
    bit<48> body;        // 6 bytes: covers the mandatory padding in ext_len=0 headers
}
                   ...
                   inout standard_metadata_t standard_metadata)
{
    ...
    state parse_ipv6 {
	    ...
	    // 🔥 EXTENSION HEADERS (CRITICAL FIX)
            0: parse_ipv6_ext;    // Hop-by-Hop
            44: parse_ipv6_ext;   // Routing
            60: parse_ipv6_ext;   // Destination Options
            default: accept;
        }
    }

    state parse_ipv6_ext {
	// Use extract (not lookahead) so the 8-byte body is stored and the
    	// cursor is reliably advanced by a FIXED amount BMv2 can handle.
    	packet.extract(hdr.ipv6_ext.next);

	bit<8> ext_len = hdr.ipv6_ext.last.hdr_ext_len;

	// We already consumed 8 bytes via extract. Skip any EXTRA 8*ext_len bytes.
    	// For ext_len=0 (iperf typical): advance(0) — a no-op. Safe.
    	// For ext_len=1: advance(64) — skip 8 more bytes.
    	packet.advance((bit<32>)(ext_len) * 64);	

	// NOW ip_proto is reliably set to the real next protocol.
    	local_metadata.ip_proto = hdr.ipv6_ext.last.next_hdr;

    	transition select(hdr.ipv6_ext.last.next_hdr) {
            IP_PROTO_TCP: parse_tcp;
            IP_PROTO_UDP: parse_udp;
            IP_PROTO_ICMPV6: parse_icmpv6;
	    IP_PROTO_SRV6:   parse_srv6;

            // chain more extension headers
            0: parse_ipv6_ext;
            44: parse_ipv6_ext;
            60: parse_ipv6_ext;
	...
	local_metadata.ip_proto = IP_PROTO_TCP;
        transition accept;
    }
	...
	local_metadata.ip_proto = IP_PROTO_UDP;
        transition accept;
    }
...
control IngressPipeImpl (inout parsed_headers_t    hdr,
    ...
    action classify_traffic() {
    	...
    	local_metadata.traffic_class = 0; // Default Bulk

    	if (local_metadata.ip_proto == IP_PROTO_UDP) {
	    local_metadata.traffic_class = 1; // Voice
            if (local_metadata.l4_dst_port == 16w5004) {	// explcit bit-width
            	local_metadata.traffic_class = 2; // Video
            }
    	}
    }
    ...
    table qos_table {
    	key = {
	    local_metadata.traffic_class: exact;
    	}
    	actions = {
	    set_high_priority;
            set_medium_priority;
...
control DeparserImpl(packet_out packet, in parsed_headers_t hdr) {
	...
	packet.emit(hdr.ipv6_ext); // 👈 This will now emit all valid headers in the stack
